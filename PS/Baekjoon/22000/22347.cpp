...
signed main() {
    ...
    while (T--) {
        ...
        fill(sa, sa + K + 2, 0), fill(lcp, lcp + K + 2, 0);
        ...
        while (X <= K) {
            ...
            for (int i = 2; i <= K; i++)
                nxt[sa[i]] = nxt[sa[i - 1]] + (group[sa[i - 1]] == group[sa[i]] ? group[sa[i - 1] + X] < group[sa[i] + X] : group[sa[i - 1]] < group[sa[i]]);
            for (int i = 1; i <= K; i++)
                group[i] = nxt[i];
            X <<= 1;
        }

        arr[0 + K + 1].push_back(0), S[0] = 0;
        for (int i = 1; i <= K; i++) {
            rnk[sa[i]] = i, S[i] = S[i - 1] + (A[i] == 2 ? 0 : (A[i] == 0 ? 1 : -1));
            arr[S[i] + K + 1].push_back(i);
        }
        for (int i = -K - 1; i <= K; i++)
            arr[i + K + 1].push_back(K * 2);
        ...
        for (int i = 2; i <= K; i++) {
            ...
            if (V < X)
                continue;
            L = lower_bound(arr[S[X - 1] + K + 1].begin(), arr[S[X - 1] + K + 1].end(), X + Z) - arr[S[X - 1] + K + 1].begin();
            R = upper_bound(arr[S[X - 1] + K + 1].begin(), arr[S[X - 1] + K + 1].end(), V) - arr[S[X - 1] + K + 1].begin() - 1;
            if (L > R)
                continue;

            Z = max(Z, arr[S[X - 1] + K + 1][R] - X + 1);
            if ((R - L + 1) < P)
                P -= (R - L + 1);
            else {
                ans = _A.substr(X - 1, arr[S[X - 1] + K + 1][L + P - 1] - X + 1);
                break;
            }
        }
