...
signed main() {
    ...
    st.update(lower_bound(comp.begin(), comp.end(), arr[0][1]) - comp.begin() + 1, lower_bound(comp.begin(), comp.end(), arr[0][2]) - comp.begin(), arr[0][3]);

    for (int i = 1; i < N * 2; i++) {
        diff = arr[i][0] - arr[i - 1][0];
        res += diff * st.query(1, MAX);
        st.update(lower_bound(comp.begin(), comp.end(), arr[i][1]) - comp.begin() + 1, lower_bound(comp.begin(), comp.end(), arr[i][2]) - comp.begin(), arr[i][3]);
    }
